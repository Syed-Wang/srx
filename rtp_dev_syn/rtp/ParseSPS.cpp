...
static int h264_parse_sps(const unsigned char *data, unsigned int dataSize, sps_info_struct *info)
{
...
{
    if (pData == NULL || dataSize <= 12)
		return -1;

	const unsigned char* pTemp = pData;
    //跳过RTP头
    pTemp += 12;
    
    int naltype = pTemp[0] & 0x1F;
    if(naltype == 7) //SPS
    {
        sps_info_struct info = { 0x00 };
        if(h264_parse_sps(pTemp, dataSize-12, &info) > 0)
        {
            if(cb != NULL)
            {
                cb(info.width, info.height, info.fps);
            }
        }
        return 0;
    }
